...
public class GenericExceptionHandlerResolver extends ExceptionHandlerExceptionResolver {
    ...
    public GenericExceptionHandlerResolver() {
        setOrder(HIGHEST_PRECEDENCE); // to handle the error before Spring's DefaultHandlerExceptionResolver
    }
    ...
    public void afterPropertiesSet() {
        ...
        exceptionHandlerMethodResolver = new ExceptionHandlerMethodResolver(exceptionHandlerMapping.getClass());
    }
    ...
    protected ModelAndView doResolveHandlerMethodException(
            HttpServletRequest request, HttpServletResponse response, HandlerMethod handlerMethod, Exception exception) {
        if (handlerMethod != null) {
            request.setAttribute(ExceptionHandlerMapping.HANDLER_METHOD, handlerMethod.getBeanType().getSimpleName());
        }
